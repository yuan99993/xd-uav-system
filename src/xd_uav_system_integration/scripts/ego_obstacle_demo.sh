...
is_running() {
  ...
  [[ -n "$stat" && "$stat" != Z* \
     && "$args" == *"roslaunch xd_uav_system_integration ego_obstacle_demo.launch"* ]]
}
...
wait_ready() {
  ...
  local state nodes
  echo "waiting for MAVROS, estimator, manager, EGO and automatic takeoff..."
  while ((SECONDS < deadline)); do
    ...
    if grep -q '^connected: True$' <<<"$state" \
      ...
      && timeout -k 1 3 rostopic echo -n 1 /uav1/ego/bridge/healthy 2>/dev/null | grep -q '^data: True$' \
      && timeout -k 1 3 rostopic echo -n 1 /uav1/ego/system_healthy 2>/dev/null | grep -q '^data: True$' \
      ...
      echo "demo ready and airborne; MAVROS/estimator/manager/EGO chain is active"
      return 0
    fi
    sleep 1
  done

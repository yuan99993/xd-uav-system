...
近地高度后使用更低的最终下降速度。返航降落的XY返回阶段使用纯速度控制，
不同时生成机体前方的移动位置目标，避免位置与制动速度相互拉扯。
`landing/return_max_velocity`只规定速度上限，控制器依据剩余距离和多旋翼水平最大
加速度自动生成制动速度包络，并根据实际水平速度误差主动制动。只有实际位置和速度都
进入容差后，才切换为Home位置保持并执行垂直下降。控制器不会把高度目标一步跳到地面。

`landing/height_source`可设为`odom`或`distance_sensor`，对应参数分别位于
`landing/odom_cfg`和`landing/distance_sensor_cfg`。前者保留相对Home/起飞点地面的
原有行为，适用于没有下视传感器的仿真；后者仅在纵向使用经过姿态补偿的下视`Range`
建立AGL目标，使用滤波后的AGL变化率判断触地，不再依赖所选定位源的odom Z/VZ。
测距模式只把AGL误差转换到当前odom坐标表达，返航和保持的XY逻辑仍使用定位源。
数据丢失会冻结Z参考，不会退回Home高度继续盲降；AGL高度和变化率满足容差时，
`ControlCommand.landing_touchdown`会通知管理器执行停桨。

...
单机目标必须使用上面的 `ego_obstacle_demo.sh goal X Y Z`。目标必须使用 `world`，Z 会原样进入
EGO；可用高度受启动文件中的地图 Z 范围和障碍物安全边界约束。默认目标 `(6,0,1)` 已验证。
...
健康。单机目标必须使用 `world`；超出三维地图规划范围或落在障碍物内的目标可能无法生成
安全轨迹。三机 preset 演示当前不接受运行时改点。

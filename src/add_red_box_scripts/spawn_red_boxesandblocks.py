...
# (kind, position_xyz, size_xyz, yaw_deg)
ObstacleSpec = Tuple[str, Tuple[float, float, float], Tuple[float, float, float], float]
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=2,
        help="ground height used when an explicit Z is omitted (default: 2)",
    )
    parser.add_argument(
        "--margin",
        type=float,
    ...
    parser.add_argument(
        ...
        default=0.0,
        metavar="M",
        help="extra footprint clearance from search-area edges (default: 0)",
    )
    parser.add_argument(
        "--obstacle-min-spacing",
        type=float,
        default=2.0,
        metavar="M",
        help="minimum edge-to-edge clearance between tall obstacles (default: 2)",
    )
    parser.add_argument(
        "--obstacle-red-spacing",
        type=float,
        default=2.0,
        metavar="M",
        help="minimum edge-to-edge clearance between tall obstacles and red boxes (default: 2)",
    )
    parser.add_argument(
        "--obstacle-prefix",
        default="obstacle",
        help="Gazebo obstacle model-name prefix (default: obstacle)",
    )
    parser.add_argument(
        "--obstacle-start-index",
        type=int,
        default=1,
        help="first obstacle model index (default: 1)",
    )
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="delete old red-box and generated obstacle models before spawning",
    )
    parser.add_argument(
        "--dynamic",
        action="store_true",
...
def _validate_args(parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
    ...
    if not math.isfinite(args.obstacle_min_spacing) or args.obstacle_min_spacing < 0.0:
        parser.error("--obstacle-min-spacing must be finite and non-negative")
    if not math.isfinite(args.obstacle_red_spacing) or args.obstacle_red_spacing < 0.0:
        parser.error("--obstacle-red-spacing must be finite and non-negative")
    if not re.fullmatch(r"[A-Za-z][A-Za-z0-9_]*", args.obstacle_prefix):
        parser.error(
            "--obstacle-prefix must start with a letter and contain only letters, digits, or '_'"
        )
    if args.obstacle_start_index < 0:
        parser.error("--obstacle-start-index cannot be negative")
    if args.position is not None and args.obstacle_count > 0:
        parser.error(
            "--obstacle-count requires random placement with --count; "
            "explicit --position does not define an obstacle generation area"
        )
...
def _choose_polygon_indices(
    count: int,
    polygons: Sequence[Polygon2],
    generator: random.Random,
) -> List[int]:
    """Spread objects over all polygons, then weight extras by polygon area."""

    if count <= 0:
        return []
    areas = [_polygon_area(polygon) for polygon in polygons]
    if not areas or any(area <= 1e-9 for area in areas):
        raise ValueError("obstacle placement polygons are empty or invalid")

    selected: List[int] = []
    if count >= len(polygons):
        selected.extend(range(len(polygons)))
    else:
        selected.extend(generator.sample(range(len(polygons)), count))

    total_area = sum(areas)
    while len(selected) < count:
        pick = generator.uniform(0.0, total_area)
        accumulated = 0.0
        chosen = len(polygons) - 1
...
def _random_obstacles_in_polygons(
    count: int,
    polygons: Sequence[Polygon2],
    ground_z: float,
    ...
    if count <= 0:
        return []
    if not polygons:
        raise ValueError("no usable polygons are available for obstacle placement")

    for index, polygon in enumerate(polygons):
        if len(polygon) < 3 or not all(
            math.isfinite(value) for point in polygon for value in point
        ):
            raise ValueError(f"obstacle placement polygon {index} is invalid")
        if _polygon_area(polygon) <= 1e-9:
            raise ValueError(f"obstacle placement polygon {index} has zero area")

    selected_indices = _choose_polygon_indices(count, polygons, generator)
    red_radius = math.hypot(float(red_box_size[0]) / 2.0, float(red_box_size[1]) / 2.0)
    occupied: List[Tuple[float, float, float]] = []
    result: List[ObstacleSpec] = []
    ...
    for selected_index in selected_indices:
        ...
        clearance = radius + float(margin)
        polygon = polygons[selected_index]
        ...
        if bounds[0] >= bounds[1] or bounds[2] >= bounds[3]:
            raise ValueError(
                f"search polygon {selected_index} is too small for a {kind} "
                f"with footprint {footprint_x:g} x {footprint_y:g}"
            )

        for _ in range(8000):
            x = generator.uniform(bounds[0], bounds[1])
            y = generator.uniform(bounds[2], bounds[3])
            ...
            if any(
                ...
                for old_x, old_y, old_radius in occupied
            ):
                continue
            if any(
                math.hypot(x - red[0], y - red[1])
                < radius + red_radius + red_obstacle_spacing
                for red in red_box_positions
            ):
                continue

            position = (x, y, centre_z)
            result.append((kind, position, size, yaw_deg))
...
def _obstacle_model_sdf(name: str, size: Sequence[float], static: bool = True) -> str:
    """Gazebo box primitive using the simulator's default visual material."""
...
def _spawn(
    ...
    if delete_model is not None:
        try:
            ...
            for old_name in world.model_names:
                if not (
                    red_pattern.fullmatch(old_name)
                    or obstacle_pattern.fullmatch(old_name)
                ):
                    continue
            ...
            print(f"Could not clear old generated models: {error}", file=sys.stderr)
            return 2
    ...
    for offset, obstacle in enumerate(obstacles):
        kind, position, size, yaw_deg = obstacle
        name = (
            f"{args.obstacle_prefix}_{kind}_"
            f"{args.obstacle_start_index + offset}"
        )
        try:
            response = spawn_model(
                name,
                _obstacle_model_sdf(name, size, static=True),
                "",
                _pose(position, yaw_deg),
                args.reference_frame,
            )
        except (rospy.ServiceException, rospy.ROSInterruptException) as error:
            failures += 1
...
def main() -> int:
    ...
    polygons: Optional[List[Polygon2]] = None
    ...
    try:
        if args.position is not None:
            ...
            polygons = _rectangle_polygons(areas)

        obstacles: List[ObstacleSpec] = []
        if args.obstacle_count > 0:
            if polygons is None:
                raise ValueError(
                    "no random placement polygons are available for tall obstacles"
                )
            obstacles = _random_obstacles_in_polygons(
                args.obstacle_count,
                polygons,
                args.ground_z,
    ...
    if args.dry_run:
        for offset, position in enumerate(positions):
            name = f"{args.prefix}_{args.start_index + offset}"
            print(
                f"{name}: x={position[0]:.3f}, y={position[1]:.3f}, "
                f"z={position[2]:.3f}"
            )
        for offset, obstacle in enumerate(obstacles):
            kind, position, size, yaw_deg = obstacle
    ...
    return _spawn(args, positions, obstacles)
